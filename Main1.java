...
public class Main1 {
	...
	//static List<String> noMonth = new LinkedList<>();
    ...
    static List<Integer> arrOfMonths = new LinkedList<>();
    
    static int fullDates[];
    ...
    public static void main(String[]args)
    {
        ...
        for(int i=0;i<fullWords.size();i++)
        {
        	regexDates(4,"January|February|March|April|May|June|July|August|September|October|November|December",step2Words.get(i));
        }
        ...
        //arrofMonths
        fullDates = new int[arrofDates.length];
        ...
        //Add years and months to form more precise dates.
       // static int fullDates[];
        for(int i=0;i<fullDates.length;i++)
        {
        	String date="";
        	try{
        	date=""+arrofDates[i]+arrOfMonths.get(i);}
        	catch(Exception e)
        	{
        		date=""+arrofDates[i]+"00";
        	}//Its not the best way to do this but it works for now i will fix it sometime
        	
        	System.out.println(date);
        	fullDates[i]=Integer.parseInt(date);
        }
        
        sortedDates = sort(fullDates);
        
        
        for(int i=0;i<sortedDates.length;i++)//REFERENCE -TO MAINTAIN MEANING FOR EVERY DATE.
        {
        	//Search in unsorted array where is the sorted information.
        	for(int j=0;j<sortedDates.length;j++)
        	{
        		//arrofRefs[i] = ;
        		if(fullDates[j]==sortedDates[i])
        		{
        			arrofRefs[i]=j;
        			break;
        		}
    	  ...
    	  Pattern checkRegex = Pattern.compile(regex);
    	    ...
    	    while(matchRegex.find())
    	    {
    	        if(matchRegex.group().length() != 0)
    	        {
    	        	...
    	        	{
    	        		arrOfMonths.add(getMonth(matchRegex.group().trim()));
